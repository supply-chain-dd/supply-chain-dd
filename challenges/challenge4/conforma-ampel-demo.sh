...
pe "bat ${CONFORMA_POLICY_DIR}/sbom-baseline-check.rego"
...
p "bat ${WORK_DIR}/ampel-policies/verify-build-artifacts.hjson"
bat ${CHALLENGE3_SECURITY}/ampel-policies/verify-build-artifacts.hjson
...
if command -v ampel &>/dev/null; then
    ...
    p "ampel verify \\
  ${IMAGE_DIGEST} \\
  --policy ${WORK_DIR}/ampel-policies/verify-build-artifacts.hjson \\
