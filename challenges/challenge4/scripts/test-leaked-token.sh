...
# Test app list
echo "   Running: argocd app list --auth-token=\$ARGOCD_AUTH_TOKEN --server localhost:30443 --insecure --grpc-web"
if argocd app list --auth-token="$ARGOCD_AUTH_TOKEN" --server localhost:30443 --insecure --grpc-web > /dev/null 2>&1; then
...
if argocd app get recipe-api-production --auth-token="$ARGOCD_AUTH_TOKEN" --server localhost:30443 --insecure --grpc-web > /dev/null 2>&1; then
...
if echo y | argocd login localhost:30443 --username admin --password admin123 --insecure --grpc-web > /dev/null 2>&1; then
