...
pe "SSL_CERT_FILE=${CA_CERT} ${SYFT_CMD} scan localhost:${REGISTRY_NODE_PORT}/golang:1.25-alpine -o spdx-json --file ${WORK_DIR}/sbom-poisoned.json"
...
pe "echo \"Couches image empoisonnée: \$(podman inspect localhost:${REGISTRY_NODE_PORT}/golang:1.25-alpine --format '{{len .RootFS.Layers}}')\""
...
pe "skopeo inspect --config --tls-verify=false docker://localhost:${REGISTRY_NODE_PORT}/golang:1.25-alpine | jq '.config.Entrypoint'"
# p "9. Recherche de fichiers suspects dans l'image empoisonnée"
# pe "podman run --rm --entrypoint '' localhost:${REGISTRY_NODE_PORT}/golang:1.25-alpine ls -la /usr/local/bin/backdoor.sh /etc/profile.d/init.sh 2>/dev/null || echo 'Fichiers non trouvés'"
# pe "podman run --rm --entrypoint '' localhost:${REGISTRY_NODE_PORT}/golang:1.25-alpine cat /usr/local/bin/backdoor.sh 2>/dev/null || echo 'backdoor.sh non trouvé'"
