...
kubectl delete job generate-baseline-from-hub -n ctf-challenge --ignore-not-found 2>/dev/null
pe "kubectl create -f ${SCRIPT_DIR}/tekton-patched/jobs/generate-baseline-from-hub-job.yaml"
kubectl wait --for=condition=ready pod -l job-name=generate-baseline-from-hub -n ctf-challenge --timeout=120s 2>/dev/null
BASELINE_POD=$(kubectl get pods -n ctf-challenge -l job-name=generate-baseline-from-hub -o jsonpath='{.items[0].metadata.name}')
pe "kubectl logs -f ${BASELINE_POD} -n ctf-challenge"
kubectl logs ${BASELINE_POD} -n ctf-challenge | sed -n '/^===BASELINE_JSON_START===/,/^===BASELINE_JSON_END===/{ //!p; }' > ${WORK_DIR}/baseline-packages.json
...
pe "podman tag golang:1.25-alpine localhost:${REGISTRY_NODE_PORT}/golang:1.25-alpine"
pe "podman push localhost:${REGISTRY_NODE_PORT}/golang:1.25-alpine"

pe "podman pull alpine:3.20"
pe "podman tag alpine:3.20 localhost:${REGISTRY_NODE_PORT}/alpine:3.20"
pe "podman push localhost:${REGISTRY_NODE_PORT}/alpine:3.20"

p "Conseil : Activez les tags immuables sur votre registre si disponible"

p "5. Récupérer les digests des images propres"
pe "GOLANG_DIGEST=\$(skopeo inspect  docker://localhost:${REGISTRY_NODE_PORT}/golang:1.25-alpine | jq -r .Digest)"
pe "echo \"golang digest: \${GOLANG_DIGEST}\""

pe "ALPINE_DIGEST=\$(skopeo inspect  docker://localhost:${REGISTRY_NODE_PORT}/alpine:3.20 | jq -r .Digest)"
pe "echo \"alpine digest: \${ALPINE_DIGEST}\""

p "6. Générer les SBOMs, les attacher et créer la baseline via un Job dans le cluster"
kubectl delete job generate-sbom-baseline -n ctf-challenge --ignore-not-found 2>/dev/null
pe "kubectl create -f ${SCRIPT_DIR}/tekton-patched/jobs/generate-sbom-baseline-job.yaml"
kubectl wait --for=condition=ready pod -l job-name=generate-sbom-baseline -n ctf-challenge --timeout=300s 2>/dev/null
BASELINE_POD=$(kubectl get pods -n ctf-challenge -l job-name=generate-sbom-baseline -o jsonpath='{.items[0].metadata.name}')
pe "kubectl logs -f ${BASELINE_POD} -n ctf-challenge"
...
echo "http://ctf-admin:CTFSecurePass123%21@localhost:30002" > ${WORK_DIR}/.git-credentials
...
pe "SSL_CERT_FILE=${CA_CERT} oras discover \
  ...
  localhost:${REGISTRY_NODE_PORT}/recipe-api:v3.0"

p "15. Vérifier les résultats du pipeline"
pe "kubectl get pipelinerun ${LATEST_PR_NAME} -n ctf-challenge -o jsonpath='{.status.results}' | jq ."
...
sleep 10
IMAGE_DIGEST=$(kubectl get pipelinerun ${LATEST_PR_NAME} -n ctf-challenge -o jsonpath='{.status.results[?(@.name=="IMAGE_DIGEST")].value}' 2>/dev/null)
...
pe "cosign initialize --mirror=http://localhost:30007 --root=${TUF_ROOT_FILE}"
rm -f "${TUF_ROOT_FILE}"

if command -v ec &>/dev/null; then
    p "16. Conforma (Enterprise Contract) — vérification de conformité"
    p "  Identités attendues :"
    p "    - Pipeline : https://kubernetes.io/namespaces/ctf-challenge/serviceaccounts/pipeline-keyless-signer"
  ...
  --images '{\"components\":[{\"name\":\"recipe-api\",\"containerImage\":\"localhost:${REGISTRY_NODE_PORT}/recipe-api:v3.0@${IMAGE_DIGEST}\"}]}' \
  --policy '{\"sources\":[{\"name\":\"sbom-baseline\",\"policy\":[\"${SCRIPT_DIR}/security/conforma-policies/\"]}]}' \
  --certificate-identity-regexp 'https://kubernetes.io/namespaces/(ctf-challenge|tekton-chains)/serviceaccounts/(pipeline-keyless-signer|tekton-chains-controller)' \
  --certificate-oidc-issuer ${OIDC_ISSUER} \
  --rekor-url http://localhost:30006 \
...
if command -v ampel &>/dev/null; then
  ...
  --collector \"coci:localhost:${REGISTRY_NODE_PORT}/recipe-api@${IMAGE_DIGEST}\" \
  ...
  localhost:${REGISTRY_NODE_PORT}/recipe-api@${IMAGE_DIGEST} 2>&1 | head -5 || true"
    ...
    p "    --signer 'sigstore:::${OIDC_ISSUER}:::https://kubernetes.io/namespaces/ctf-challenge/serviceaccounts/pipeline-keyless-signer'"
else
